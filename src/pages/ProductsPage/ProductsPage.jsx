    ...
    const onScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        for (const id of ids) {
          ...
          if (!el) continue
          if (el.getBoundingClientRect().top + window.scrollY <= threshold) cur = id
        }
        setActive(cur)
        ticking = false
      })
...
function Sidebar({ activeId, filter, onFilter, elevators }) {
  ...
  const scrollTo = id => {
    ...
    window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - navH - 6, behavior: 'smooth' })
  }
  ...
  return (
    <aside className="sidebar" aria-label="Elevator navigation">
        ...
        {elevators.map(elev => {
          ...
          return (
            <a
              ...
              onClick={e => { e.preventDefault(); scrollTo(elev.id) }}
              aria-current={isActive ? 'page' : undefined}
            >
              <span className="sidebar-item-dot" style={{ background: isActive ? dot : 'transparent' }} />
              ...
              <span className="sidebar-item-body">
                ...
                <span className="sidebar-item-ctx">{elev.context.split('·')[0].trim()}</span>
              </span>
      ...
      <div className="sidebar-cta">
        ...
        <p className="sidebar-cta-body">Our engineers will size the right elevator for your building.</p>
        <Link to="/contact" className="sidebar-cta-btn">Get a Free Quote</Link>
      </div>
    </aside>
  )
}
...
function ElevatorSection({ elev }) {
  ...
  return (
    <article className="elev-section" id={elev.id} aria-labelledby={`${elev.id}-h`}>
      ...
      <div className="elev-hero">
        <div className="elev-img-box">
          ...
          <span className="elev-img-num" aria-hidden="true">{elev.number}</span>
        </div>
        <div className="elev-meta">
          <div className="elev-meta-tag">{elev.context}</div>
          <h2 className="elev-meta-title" id={`${elev.id}-h`}>{elev.title}</h2>
      ...
      {/* Spec strip */}
      <div className="elev-specs" aria-label={`${elev.title} key specifications`}>
        {elev.specs.map(s => (
      ...
      {(hasDetail || hasTables) && (
        <>
          <button
            ...
            aria-expanded={open}
          >
            <span>{open ? 'Hide Technical Details' : 'View Technical Specifications'}</span>
                    ...
                    <div className="elev-detail-panel">
                      ...
                      <ul className="elev-feature-list">
                        {elev.features.map((f, i) => (
                          <li key={i}>
                            <span className="elev-feature-check">✓</span>{f}
                          </li>
                        ))}
                      </ul>
                    </div>
...
export default function ProductsPage() {
  ...
  const visible = useMemo(
    () => filter === 'all' ? ELEVATORS : ELEVATORS.filter(e => e.categories.includes(filter)),
    [filter]
  )
  ...
  return (
    <main>
      <PageHero
        ...
        chips={['Passenger', 'Hospital', 'Goods', 'Capsule', 'Car', 'Home', 'MRL', 'Hydraulic']}
        dataBg="PRODUCTS"
      />
